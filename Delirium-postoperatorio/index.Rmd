...
my_summary <-
  with(dt,
       list("EtÃ  (anni)" = tab_summary(Eta..anni),
...
p_vals_freq<-
  ...
  do.call(c, .)
# base<-with(dt,
#   list("Kurtosis etÃ "=anscombe.test(Eta..anni),
#        "Skewness etÃ "=agostino.test(Eta..anni),
...
# 
# base_plot<-with(dt,
#   list("Histogram etÃ "=hist(Eta..anni),
#        "QQPlot etÃ "=qqnorm(Eta..anni),
...
# 
#    Predicted mortality =   e (Î²0 + Ã¥bi Xi) / 1+ e (Î²0 + Ã¥bi Xi)
# 
 # Î²o is the constant of the logistic regression equation = -4.789594
 # 
 # Î²i is the coefficient of the variable Xi in the logistic regression equation provided in the table below.
 # 
...
Le frequenze a due categorie sono state analizzate con test $\chi2$, le frequenze a piÃ¹ categorie con il test non parametrico di Kruskal-Wallis, le differenze di distribuzione sono state valutate con il test di Mann-Whitney.  
...
```{r plot_eta, warning=FALSE, echo=FALSE, opts.label='myfigures', fig.cap="Distribuzione EtÃ  - il punto rosso indica la media"}
ggplot(data=dt, aes(x=Delirium,fill=Delirium,Eta..anni))+
  geom_boxplot(colour=NA, show.legend = F)+
  geom_boxplot(show.legend = F)+
  labs(title="EtÃ ", x="Delirium", y="EtÃ  (anni)", fill="")+
...
I modelli di regressione logistica identificati utilizzano come predittori l'etÃ , l'EuroSCORE, il tempo trascorso con BIS<40, il tempo trascorso con SR>0, la necessitÃ  di trasfusioni intraoperatorie e l'utilizzo di vasopressori.  
I coefficienti di regressione ottenuti nei modelli rappresentano la variazione del logit ($logit(p(x)) = log (\frac{p(x)}{1 â p(x)}) = log(OR) = \alpha+\beta x$) per ogni cambiamento unitario del predittore. Visto che il logit non Ã¨ intuitivo, di solito si valuta l'effetto del predittore sull'esponenziale del coefficiente di regressione - l'odds ratio.  
...
eq<-paste0("$logit(Delirium) = ", round(tb$estimate[1],2)," + ",
           round(tb$estimate[2],4)," * EtÃ  + ",
           round(tb$estimate[3],4)," * EuroSCORE$")
knitr::kable(tb[,.(" "= c("Intercetta","EtÃ  (anni)","EuroSCORE II"), 
...
E' possibile calcolare anche la probabilitÃ  di sviluppare l'evento come  

$p(x)=\frac{e^{\alpha+\beta}}{1+e^{\alpha+\beta}}$
  
Vengono di seguito presentate le probabilitÃ  predette di sviluppare delirium post intervento in funzione del tempo passato con BIS <40, in tre classi di etÃ , e in funzione dell'EuroSCORE, sempre in tre classi di etÃ .  

```{r plot_log-BIS, message=FALSE, warning=FALSE, echo=FALSE, opts.label='myfigures', fig.cap="ProbabilitÃ  di sviluppare delirium post-operatorio - Tempo BIS<40"}
plot_model(fm3_l,type="eff",terms = c("BIS.40..min.",
                                      "Eta..anni [40,50,60,70,80]"),
           title = "ProbabilitÃ  predette di svillupare delirium",
           axis.title = c("Tempo BIS<40 (min)","ProbabilitÃ "),
           legend.title = "Classi d'etÃ ", ci.lvl = NA)+
  scale_colour_discrete()+
  theme(legend.position="bottom")
```
  
```{r plot_log-escore, message=FALSE, warning=FALSE, echo=FALSE, opts.label='myfigures', fig.cap="ProbabilitÃ  di sviluppare delirium post-operatorio - EuroSCORE II"}
plot_model(fm3_l,type="eff",terms = c("EuroSCORE.II...LOGISTICO",
                                      "Eta..anni [40,50,60,70,80]"),
           title = "ProbabilitÃ  predette di svillupare delirium",
           axis.title = c("EuroSCORE II (logistico)","ProbabilitÃ "),
           legend.title = "Classi d'etÃ ", ci.lvl = NA)+
  scale_colour_discrete()+
  theme(legend.position="bottom")
```
...
```{r plot_log-sr0, message=FALSE, warning=FALSE, echo=FALSE, opts.label='myfigures', fig.cap="ProbabilitÃ  di sviluppare delirium post-operatorio - Tempo con SR > 0"}
plot_model(fm3_l_sr,type="eff",terms = c("SR.0..min.",
                                      "Eta..anni [40,50,60,70,80]"),
           title = "ProbabilitÃ  predette di svillupare delirium",
           axis.title = c("Tempo con SR>0 (min)","ProbabilitÃ "),
           legend.title = "Classi d'etÃ ", ci.lvl = NA)+
  scale_colour_discrete()+
  theme(legend.position="bottom")
```
...
knitr::kable(tb[,.(" "= c("Intercetta","EtÃ  (anni)", 
                          "Tempo BIS<20 (min)"), 
                   "OR"=exp(estimate),"OR.lower"=exp(conf.low),
                   "OR.higher"=exp(conf.high),
                   "$\\beta$"=estimate,
...
```{r plot_log-bis20, message=FALSE, warning=FALSE, echo=FALSE, opts.label='myfigures', fig.cap="ProbabilitÃ  di sviluppare delirium post-operatorio - Tempo con bis < 20"}
plot_model(fm_fasullo,type="eff",terms = c("BIS..20",
                                      "Eta..anni [40,50,60,70,80]"),
           title = "ProbabilitÃ  predette di svillupare delirium",
           axis.title = c("Tempo con BIS<20 (min)","ProbabilitÃ "),
           legend.title = "Classi d'etÃ ")+
  scale_colour_discrete()+
  theme(legend.position="bottom")
```
